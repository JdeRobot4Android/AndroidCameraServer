...
public class CameraPreview extends SurfaceView implements
	...
	public CameraPreview(Context context, Camera camera,
			PreviewCallback previewCb, AutoFocusCallback autoFocusCb, int wid,
			int hei) {
		super(context);
		mCamera = camera;
		...
		bytesperPixel = ((float) ImageFormat.getBitsPerPixel(mParameters
				.getPreviewFormat())) / 8;
		...
		for (int i = 0; i < sizes.size(); i++) {
			...
			Log.i("Resolution", "Width: " + result.width + " x Height: "
					+ result.height);
